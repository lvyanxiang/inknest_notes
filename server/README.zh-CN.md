...
冲突记录会保存提交版本和当前版本的完整 JSON、哈希、提交时的 `baseRevision`、当前服务
端 Revision、来源设备、稳定的 `copyResourceId` 和 `conflictOf` 来源关系。笔记本显示为
`原名称（冲突副本）`，无独立标题的页面显示为 `第 N 页（冲突副本）`；设备与发生时间
作为附加信息，不参与标题和资源身份判断。
...
- `keep_original`：关闭冲突，双方快照仍保留用于恢复。
- `use_conflict`：只有原资源在冲突发生后没有再次变化，才把提交快照写成原资源的新
  Revision。
- `keep_both`：使用预留副本 ID 创建新笔记本或追加页面，并通过 `conflictOf` 指向原资源。
