...
- [ ] 新设备全量 bootstrap 后切换至增量同步。
  ...
  - [ ] 为标题、归档、文件夹、页面结构和画布背景设计结构变更合同；现有
    `/sync/commit` 只更新完整 content，客户端不得伪装这些字段已同步。
  ...
  - [ ] 接入 `POST /sync/conflicts/{conflictId}/resolve` 的保留原版本、使用冲突版本和
    两个都保留操作，并在服务端确认后协调本地资源与待处理列表。
- [ ] 恢复前快照和失败回滚。
